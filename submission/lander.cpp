...

{
  ...
  // YOUR CODE HERE
  width = LANDER_WIDTH;
  height = s * (max.y - min.y);

  glGenVertexArrays(1, &VAO);       //Review VAO Lecture Material to ensure correct
...
{
  // YOUR CODE HERE
    float x = position.x;   //Get coordinates of lander
    float y = position.y;
    worldToViewTransform = worldToViewTransform * translate(x, y, 0) * rotate(orientation, vec3(0, 0, 1));  //Translate to its proper coordinates
    glBindVertexArray(VAO);
