...

{
  ...
  // YOUR CODE HERE
  vec3 segment = vec3(segHead.x - segTail.x, segHead.y - segTail.y, 0);   //Vector that stores current segment
  vec3 normalizedS = 1 / sqrt(segment.x * segment.x + segment.y * segment.y) * segment;
  vec3 pos = position - segTail;
  float proj = pos * normalizedS;      //Projection of position onto segment (dot product)
  if (proj < 0) {                      //Outside range, return tail (behind the tail)
      return segTail;
  }
  if (proj > 1) {                      //Outside range, return head (in front of the head)
      return segHead;
  }

  return proj * segment + segTail;     //Position must be above segment and thus return distance + start of segment
}

float Landscape::getAltitude(int segIdx, vec3 cPos, float height) {
    float xBegin = landscapeVerts[2 * segIdx];      
    float xFinish = landscapeVerts[2 * (segIdx + 1)]; 
    float yBegin = landscapeVerts[2 * segIdx + 1];
    float yFinish = landscapeVerts[2 * (segIdx + 1) + 1];
    float ySeg = (yFinish- yBegin) / (xFinish - xBegin) * (cPos.x - xBegin) + yBegin;
    float alt = cPos.y - height * 0.5 - ySeg;
    return alt;
}
int Landscape::findLandingSegment(vec3 cPos) {      //Used for landing
    for (int i = 0; i < numVerts - 1; i++) {
        int xBegin = landscapeVerts[2 * i];
        int xFinish = landscapeVerts[2 * (i + 1)];
        if (cPos.x > xBegin && cPos.x < xFinish) {     //Ensure x is within begin and finish
            return i;
        }
    }
    return 0;
