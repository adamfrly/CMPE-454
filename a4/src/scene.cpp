...
<<<<<<< Updated upstream:a4/a4/src/scene.cpp
...
{
  // Terminate the ray?
>>>>>>> Stashed changes:a4/src/scene.cpp
...

{
    ...
    result = raytrace(eye->position, dir, 0, -1, -1, vec3(1, 1, 1));
...
=======
    ...
    float jitterValX;
    float jitterValY;
    float fnumPixelSamples = (float)numPixelSamples;

    for (int i = 0; i < numPixelSamples; i++) {
        for (int j = 0; j < numPixelSamples; j++) {
            if (jitter) {
                jitterValX = 2.0 * randIn01() - 1.0 /fnumPixelSamples; // random number in [-1/2*numPixelSamples, 1/2*numPixelSamples]
                jitterValY = 2.0 * randIn01() - 1.0 / fnumPixelSamples;
                dir = (llCorner + (x + (2.0 * i + 1.0) / (2.0 * fnumPixelSamples) + jitterValX) * right + (y + (2.0 * j + 1.0) / (2.0 * fnumPixelSamples) + jitterValY) * up).normalize();
            }
            else {
                dir = (llCorner + (x + (2.0 * i + 1.0) / (2.0 * fnumPixelSamples)) * right + (y + (2.0 * j + 1.0) / (2.0 * fnumPixelSamples)) * up).normalize();  
            }
            subresult = subresult + raytrace(eye->position, dir, 0, -1, -1, vec3(1.0, 1.0, 1.0));
        }
    }

    result = (1.0 / (fnumPixelSamples * fnumPixelSamples)) * subresult;
>>>>>>> Stashed changes:a4/src/scene.cpp
